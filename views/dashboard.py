...
"""
Módulo de Dashboard.
Saldrán próximas citas, total clientes...
"""
...
def vista_dashboard(parent):
    ...
    try:
        # CONTAR CLIENTES Y CITAS TOTALES
        session = Session()
        ...
        citas_hoy = session.query(Cita).filter(
            ...
            Cita.fecha < datetime(hoy.year, hoy.month, hoy.day) + timedelta(days=1)
        ).count()
        ...
        tabla_citas = tb.Treeview(frame_tabla_citas, columns=columnas_citas, show="headings")

        tabla_citas.heading("Cliente", text="Cliente")
        tabla_citas.heading("Fecha", text="Fecha")
        tabla_citas.heading("Hora", text="Hora")
        tabla_citas.heading("Duración", text="Duración")

        tabla_citas.column("Cliente", width=150, anchor="center")
        tabla_citas.column("Fecha", width=100, anchor="center")
        tabla_citas.column("Hora", width=80, anchor="center")
        tabla_citas.column("Duración", width=80, anchor="center")

        # OBTENER PRÓXIMAS CITAS ORDENADAS POR FECHA
        session_citas = Session()
        citas_proximas = session_citas.query(Cita).filter(
            Cita.fecha >= datetime.now()
        ).order_by(Cita.fecha).limit(5).all()

        for cita in citas_proximas:
            tabla_citas.insert("", "end", values=(
        ...
        tb.Label(frame, text=f"Error al cargar dashboard: {e}", foreground="red").pack(pady=20)